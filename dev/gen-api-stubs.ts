...
interface DocField {
  ...
  EnumValue?: number; // present when parent DocTable.Type === "Enumeration"
}
...
function buildEnumRegistry(allModules: DocModule[]): Set<string> {
  const enums = new Set<string>();
  for (const mod of allModules) {
    for (const tbl of toArr(mod.Tables)) {
      if (tbl.Type === "Enumeration" && tbl.Name) {
        enums.add(tbl.Name);
      }
    }
  }
...
const TS_SCALAR_MAP: Record<string, string> = {
  ...
  BigUInteger: "number",
  luaIndex: "number",
  uiRect: "number", // WoW UI coordinate scalar
  WOWGUID: "string",
  FramePoint: "string",
  string: "string",
  cstring: "string",
...
function mapTsType(
  ...
  if (type === "table") {
    if (innerType) {
      const inner = mapTsType(innerType, undefined, structs, enums);
      return `${inner}[]`;
    }
    return "unknown[]";
  }
  if (structs.has(type)) return type;
  if (enums.has(type)) return "number";
  const scalar = TS_SCALAR_MAP[type];
  if (scalar) return scalar;
  return "unknown";
}

function generateTypesContent(allModules: DocModule[]): string {
  const structs = buildStructRegistry(allModules);
  const enums = buildEnumRegistry(allModules);
  ...
  for (const mod of allModules) {
    for (const tbl of toArr(mod.Tables)) {
      if (tbl.Type !== "Structure" || !tbl.Name || seen.has(tbl.Name)) continue;
      seen.add(tbl.Name);

      const fields = toArr(tbl.Fields);
      lines.push(`export interface ${tbl.Name} {`);
      for (const field of fields) {
        const tsType = mapTsType(field.Type, field.InnerType, structs, enums);
        const opt = field.Nilable ? "?" : "";
...
function generateEventsContent(allModules: DocModule[]): string {
  const eventSet = new Set<string>();
  for (const mod of allModules) {
    for (const evt of toArr(mod.Events)) {
      const name = evt.LiteralName ?? evt.Name;
      if (name) eventSet.add(name);
    }
  }
  const sorted = [...eventSet].sort();

  const entries = sorted.map((name) => `  ${name}: "${name}",`);
...
function generateEnumContent(allModules: DocModule[]): string {
  const seen = new Set<string>();
  const luaLines: string[] = ["Enum = {}"];

  for (const mod of allModules) {
    for (const tbl of toArr(mod.Tables)) {
      if (tbl.Type !== "Enumeration" || !tbl.Name || seen.has(tbl.Name)) continue;
      seen.add(tbl.Name);

      const fields = toArr(tbl.Fields).filter((f) => typeof f.EnumValue === "number");
      if (fields.length === 0) continue;

      const entries = fields.map((f) => `${f.Name} = ${f.EnumValue}`).join(", ");
      luaLines.push(`Enum.${tbl.Name} = { ${entries} }`);
    }
  }
...
function generateIndex(stubsRoot: string): string {
  ...
  const hasEnum = fs.existsSync(path.join(stubsRoot, "_Enum.ts"));
  ...
  if (hasEnum) lines.push('import { _Enum } from "./_Enum.js";');
  ...
  if (retail.length > 0) {
    ...
    lines.push(`const _retailLua = _pre + ${enumPrefix}[${parts}].join("\\n");`);
  } else {
    lines.push(`const _retailLua = _pre + ${hasEnum ? "_Enum" : '""'};`);
  }
  ...
  // Re-export events constants if generated
  if (fs.existsSync(path.join(stubsRoot, "_Events.ts")))
    lines.push('export type { WowEventName } from "./_Events.js";');
...
async function run(): Promise<void> {
  ...
  if (flavor === "retail") {
    const typesContent = generateTypesContent(allModules);
    const typesFile = path.join(outDir, "types.ts");
    const typesResult = await writeIfChanged(typesFile, typesContent);
    console.log(`Types: ${path.relative(PROJECT_ROOT, typesFile)} (${typesResult})`);

    const eventsContent = generateEventsContent(allModules);
    const eventsFile = path.join(outDir, "_Events.ts");
    const eventsResult = await writeIfChanged(eventsFile, eventsContent);
    console.log(`Events: ${path.relative(PROJECT_ROOT, eventsFile)} (${eventsResult})`);

    const enumContent = generateEnumContent(allModules);
    const enumFile = path.join(outDir, "_Enum.ts");
    const enumResult = await writeIfChanged(enumFile, enumContent);
    console.log(`Enum: ${path.relative(PROJECT_ROOT, enumFile)} (${enumResult})`);
  }
