...
import { shellExtractInterface, shellExtractMissing, shellGenAtlas } from "./extractor.js";
import {
  ADDON_NAMES,
  SHARED_ADDON_NAMES,
  clearRegistryCache,
...
export class AssetService {
   ...
   * If the manifest is absent and dev/gen-atlas.mjs is found in the workspace,
   * runs the generator (which downloads from wago.tools).
   *
   * Returns true if the manifest was newly generated (caller should re-render).
   * Returns false if the manifest already existed, the script was not found, or
   * generation failed.
   *
  ...
  async ensureAtlasManifest(): Promise<boolean> {
    ...
    // shellGenAtlas checks for the listfile and silently skips if absent.
    // Only log "generating" here if the listfile is already present so we
    // don't emit the message on every render while extraction is still running.
   ...
   * Generated by dev/gen-atlas.mjs and version-tagged alongside textures.
   */
