...
//
// The rulers sit below the #status-bar (STATUS_BAR_H px tall). The left ruler
// canvas starts at viewport y = STATUS_BAR_H so its canvas coordinates are offset
// by that amount: adjustedOriginY = rect.top - STATUS_BAR_H.
//
...
export const RULER_SIZE = 20; // px — thickness of each ruler strip
export const STATUS_BAR_H = 20; // px — height of the #status-bar above the rulers

const LABEL_INTERVAL = 100; // WoW px between numeric labels
const TICK_MAJOR = 50; // WoW px — major tick (half height/width)
const TICK_MINOR = 10; // WoW px — minor tick (third height/width)

const BG = "#1a1a1a";
const BORDER = "#2a2a2a";
const TICK_MAJOR_COLOR = "#666";
const TICK_MINOR_COLOR = "#3a3a3a";
const LABEL_COLOR = "#c8c8c8";
// sans-serif renders noticeably crisper than monospace at 9 px
const FONT = "9px system-ui,sans-serif";
// dark halo makes labels legible regardless of what the canvas shows underneath
 ...
 */
export function updateRulers(wowViewportEl: HTMLElement, scale: number): void {
  ...
  const leftH = vpH - STATUS_BAR_H; // left ruler CSS height

  // Top ruler: full viewport width, positioned below the status bar (CSS: top:STATUS_BAR_H).
  // Canvas (0,0) = viewport (0, STATUS_BAR_H). X coordinates are identical to viewport.
  topCanvas.width = Math.round(vpW * dpr);
  topCanvas.height = Math.round(RULER_SIZE * dpr);
  drawHorizontal(topCanvas, dpr, rect.left, scale, vpW);

  // Left ruler: full height below the status bar (CSS: top:STATUS_BAR_H; bottom:0).
  // Canvas y=0 = viewport y=STATUS_BAR_H, so all originY values need the offset subtracted.
  leftCanvas.width = Math.round(RULER_SIZE * dpr);
  leftCanvas.height = Math.round(leftH * dpr);
  drawVertical(leftCanvas, dpr, rect.top - STATUS_BAR_H, scale, leftH);
}
...
function drawHorizontal(
  ...
  const firstTick = Math.floor(wowXStart / TICK_MINOR) * TICK_MINOR;
  const lastTick = Math.ceil(wowXEnd / TICK_MINOR) * TICK_MINOR;

  ctx.font = FONT;
  ctx.textBaseline = "top";
  ctx.textAlign = "left";

  for (let wx = firstTick; wx <= lastTick; wx += TICK_MINOR) {
    ...
    const isMajor = wx % TICK_MAJOR === 0;
    const isLabel = wx % LABEL_INTERVAL === 0;
    const tickH = isMajor ? Math.floor(H / 2) : Math.floor(H / 3);

    ctx.fillStyle = isMajor ? TICK_MAJOR_COLOR : TICK_MINOR_COLOR;
...
function drawVertical(
  ...
  const firstTick = Math.floor(wowYStart / TICK_MINOR) * TICK_MINOR;
  const lastTick = Math.ceil(wowYEnd / TICK_MINOR) * TICK_MINOR;

  ctx.font = FONT;

  for (let wy = firstTick; wy <= lastTick; wy += TICK_MINOR) {
    ...
    const isMajor = wy % TICK_MAJOR === 0;
    const isLabel = wy % LABEL_INTERVAL === 0;
