...
Benchmark infrastructure (suite scripts, reproducibility, regression detection, tooling, roadmap, test suite timings) is in **[docs/measurements-testing.md](measurements-testing.md)**.
...
### Q1b + Q1c: Listfile filtering and SQLite index

Full results, tool inventory, and run instructions are in **[docs/measurements-sql.md](measurements-sql.md)**.

**Q1b summary (2026-05-28, full 2.17M row file):** `grep -F` wins at ~110 ms. SQLite extension approaches (sqlite-xsv, sqlean vsv) land at ~790–930 ms. `awk` is always the wrong choice. Decision: use `grep -F` in `ensureFilteredListfile()`.

**Q1c summary (2026-06-17, 169K pre-filtered rows — fair comparison):** Native CSV extension approaches (`better-sqlite3`/`node:sqlite` + `sqlean vsv` or `sqlite-xsv`) build the index in 210–266 ms — 2× faster than row-by-row JS INSERT. Point lookups: 0.011 ms/query for both `better-sqlite3` and `node:sqlite` prepared statements. `@libsql/client` is definitively out (36× on build, 2.7× on lookup). Decision: `node:sqlite` + `vsv.so` `INSERT SELECT` — zero new production deps, `vsv.so` already in `external/sqlean/`.
