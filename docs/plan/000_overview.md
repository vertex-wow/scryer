...
| ↳   | [Listfile pre-filter (rustydemon era)](backlog.md#listfile-pre-filter-rustydemon-era)                                  | ✅ Done                     | `grep -F` filter at download time; cuts rustydemon's 25–33 s CSV parse to ~2–3 s              | XS     | —          |
