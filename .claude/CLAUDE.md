...
- **Completed ↳ row** — a single row with `↳` in col 1 and `colspan="5"` on col 2, containing a ✅ prefix followed by comma-separated `<a>` links to **todo-archive.md** anchors. All completed todo items for a milestone collapse into one row — no dates or descriptions in the table.
...
- **Completing a ↳:** Remove the full pending row. Add a link to it in the completed ↳ row's `colspan="5"` cell (create that row if it doesn't exist yet). No date needed in the table — dates live in todo-archive.md. Move the full entry from todo.md to todo-archive.md.
...
**Signal to watch for:** if you write the words "future", "later", "eventually", "TODO", "not yet", or "out of scope" anywhere in a document or comment, stop and ask: does this belong in todo instead?
...
2. **`docs/plan/todo.md` second** — add a full entry with a short description, the problem it solves, a rough plan, and an effort estimate. The overview row links here. When an item is completed, move its entry to `docs/plan/todo-archive.md` and update the overview link to point at the archive.
